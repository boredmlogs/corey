...
With the async iterable approach, we can push new incoming Slack messages into the iterable while the agent is still working. Instead of queuing messages until the container exits and spawning a new container, we stream them directly into the running session.
